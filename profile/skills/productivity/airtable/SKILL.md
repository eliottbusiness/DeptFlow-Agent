...
- **Schema mutations need `schema.bases:write`.** The metadata endpoints for creating/updating tables, fields, or views can fail with `403 INVALID_PERMISSIONS_OR_MODEL_NOT_FOUND` when the token only has read scope. If that happens, stop and request a higher-scope PAT or do the schema change manually, then resume backfill.
...
- See `references/schema-mutation-pitfalls.md` for the exact 403 encountered in this session and the recommended fallback sequence.
- See `references/signal-taxonomy.md` for the 2-level LinkedIn signal taxonomy (`type_signal` + `sous_type_intention`) and the session-tested backfill mapping.
